import { Check, Clock, CreditCard, Package, PartyPopper, Truck, XCircle } from "lucide-react";
import type { OrderStatus } from "@/lib/store";

const ALL_STEPS: { status: OrderStatus; label: string; icon: typeof Clock }[] = [
  { status: "Pending Review", label: "Pending Review", icon: Clock },
  { status: "Awaiting Payment", label: "Awaiting Payment", icon: CreditCard },
  { status: "Processing", label: "Processing", icon: Package },
  { status: "Ready for Delivery", label: "Ready for Delivery", icon: Truck },
...
export function OrderStatusStepper({
  ...
  // No packaging requested -> the order never goes through "Pending Review" at all
  // (it starts at "Awaiting Payment"), so that step is omitted from the display.
  const STEPS = packagingRequested ? ALL_STEPS : ALL_STEPS.slice(1);
