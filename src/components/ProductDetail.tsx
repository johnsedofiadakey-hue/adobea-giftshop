...
import { BADGE_STYLES, isGiftCardCategory, type Product } from "@/lib/products";

const GIFT_MESSAGE_LIMIT = 100;
...
export function ProductDetail({ product }: { product: Product }) {
  ...
  const isGiftCard = isGiftCardCategory(product.category);
  const [giftMessageMode, setGiftMessageMode] = useState<"write" | "blank">("blank");
  const [giftMessage, setGiftMessage] = useState("");
  const { addToCart } = useCart();
  const router = useRouter();

  const handleAdd = () => {
    const message = isGiftCard && giftMessageMode === "write" ? giftMessage.trim() : undefined;
    addToCart(product, { color, size, quantity, giftMessage: message || undefined });
    setJustAdded(true);
        ...
        <div>
          ...
          {isGiftCard && (
            <div className="mt-6 rounded-2xl border border-cream-200 bg-cream-50 p-5">
              <p className="text-sm font-semibold text-ink-900">Gift Card Message</p>
              <div className="mt-3 flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() => setGiftMessageMode("write")}
                  className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${
                    giftMessageMode === "write"
                      ? "bg-amber-500 text-white"
                ...
                <button
                  type="button"
                  onClick={() => setGiftMessageMode("blank")}
                  className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${
                    giftMessageMode === "blank"
                      ? "bg-amber-500 text-white"
                      : "bg-white text-ink-800 hover:bg-cream-200"
                  }`}
                >
              ...
              {giftMessageMode === "write" ? (
                <div className="mt-3">
                  <textarea
                    ...
                    placeholder="Write the message you'd like us to write in your card"
                  ...
                  />
                  <p className="mt-1 text-right text-xs text-ink-700/50">
                    {giftMessage.length}/{GIFT_MESSAGE_LIMIT} characters
                  </p>
                </div>
              ) : (
                <p className="mt-3 text-sm text-ink-700/70">
                  We&apos;ll leave the card blank — you can write your own message later.
                </p>
              )}
            </div>
          )}
