      ...
      <div className="rounded-2xl border border-ink-900/8 bg-cream-50 p-6">
        ...
        {tab === "Payment" && (
          <div className="max-w-sm">
            <SecretWarning>
              Checkout charges real money through Paystack (Mobile Money &amp; Bank Transfer).
              The secret key lives only in the{" "}
              <code className="rounded bg-ink-900/5 px-1">PAYSTACK_SECRET_KEY</code> server
              environment variable — never in this form, since this `settings` doc is
              public-read. Set it via{" "}
              <code className="rounded bg-ink-900/5 px-1">
                firebase apphosting:secrets:set PAYSTACK_SECRET_KEY
              </code>
              , and add{" "}
              <code className="rounded bg-ink-900/5 px-1">
                {"{"}your-domain{"}"}/api/payments/paystack/webhook
              </code>{" "}
              as the webhook URL in the Paystack dashboard so payments confirm automatically.
            </SecretWarning>
            <form
              onSubmit={(e) => {
                e.preventDefault();
            ...
            >
              <Field
                ...
                placeholder="pk_live_... (informational only — not used by checkout)"
