...
async function sendSms(body: OrderConfirmationRequest) {
  ...
  const message = `Hi ${body.customerName}, thanks for your order #${body.orderId} (${formatPrice(body.subtotal)}) with ${body.storeName || "EA_DUBEA'S GIFT HUB"}. Track it here: ${trackingUrl(body.orderId)}`;
  ...
  const res = await fetch("https://sms.arkesel.com/api/v2/sms/send", {
    ...
    body: JSON.stringify({
      sender: body.smsSenderId || "AdubeaGift",
      message,
...
function buildOrderEmailHtml(body: OrderConfirmationRequest) {
  const storeName = body.storeName || "EA_DUBEA'S GIFT HUB";
            ...
            <tr>
              <td style="background-color:${theme.secondaryColor};padding:18px 28px;text-align:center;">
                <p style="margin:0;font-size:12px;color:#ffffff;opacity:0.85;">
                  ${storeName} — Thoughtful Gifting Made Easy
                </p>
              </td>
            </tr>
          </table>
...
async function sendEmail(body: OrderConfirmationRequest) {
  ...
  const from = body.emailFromAddress || "orders@adubeagifthub.com";
  const storeName = body.storeName || "EA_DUBEA'S GIFT HUB";
