        ...
        {order && (
          <div className="mt-8 rounded-2xl border border-cream-200 bg-cream-50 p-6">
            ...
            <div className="mt-6">
              <OrderStatusStepper
                status={order.status}
                packagingRequested={order.packagingRequested}
              />
            </div>

            {order.status === "Awaiting Payment" && order.paymentStatus === "unpaid" && (
              <a
                href={`/pay/${order.id}`}
                className="mt-4 block rounded-xl bg-amber-500 px-4 py-3 text-center text-sm font-semibold text-white transition-colors hover:bg-amber-600"
              >
                Complete Payment
              </a>
            )}
